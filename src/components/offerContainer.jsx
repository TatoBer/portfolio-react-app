...
import '../styles/tradesContainer.css'
...
export default function OfferContainer({n}) {
    ...
    function CalculateR(n) {
        const kakx = document.querySelector(`.kak${n}`).parentElement.parentElement.children[1].children[0].children[0].children[1].innerHTML;
        const keyx = document.querySelector(`.kak${n}`).parentElement.parentElement.children[1].children[0].children[1].children[1].innerHTML;
        if (n == 1) {
    ...
    <>
        <div className='trade-container'>
            ...
            <div className='trade-li-container active'>
                {
                    ofers.map(value=>(
                        <Trade eliminarTrade={eliminarTrade} lado={n} key={value.id} id={value.id} kak={value.kak} keys={value.keys} desact={value.desact}/>
                    ))
                }
            </div>
        </div>
